...
def main(start_index=0, filename=None, plot_validation=False, plot_test=True, calculate_train_metric=False):
    ...
    for test_data_point_index in range(start_index, start_index+1):
        ...
        """Calculate best amplification using validation set"""
        ...
        """Skip thresholding output, use no threshold on this data"""
        best_tau = 1
        best_score = 1
        # best_tau, best_score = threshold_util.get_best_threshold(metric_predictions_unprocessed, min=0, max=0.02, num_steps=200, metric=1)
        ...
        """Plot predictions of synthetic test set"""
        if plot_test:
        ...
        for i in range(len(metric_predictions_unprocessed)):
            metric_predictions.append(
                np.argmax(metric_predictions_unprocessed[i] * np.array([[[1, best_tau]]]), axis=-1))
...
if __name__ == '__main__':
    main(start_index=0, filename="results/SciLife.csv", plot_validation=True, plot_test=True, calculate_train_metric=False)
