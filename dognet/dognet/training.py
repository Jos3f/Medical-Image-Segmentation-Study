...
def create_generator1C(data, labels, s=0 , size=(64, 64), n=10):
    """
    ...
    def f(n=n, size=size, s=s):
        ...
        d = np.zeros((n, 1, size[0], size[1]))
        l = np.zeros((n, 1, size[0] - 2 * s, size[1] - 2 * s))
        rot = np.random.randint(0, 1, n)
        flip = np.random.randint(0, 1, n)
        for i in range(n):
            index = np.random.randint(0, len(data))
            x = np.random.randint(0, data[index].shape[0] - size[0])
            y = np.random.randint(0, data[index].shape[1] - size[1])
...
def soft_dice_loss(y_pred, y_true, epsilon=1e-6):
    ...
    denominator = torch.sum(y_pred.pow(2) + y_true.pow(2))
...
def train_routine(detector,
    ...
    for i in range(n_iter):
        ...
        p, _ = detector(vx)
        optimizer.zero_grad()
        l = floss(p[:, :, margin:-margin, margin:-margin], vy[:, :, margin:-margin, margin:-margin])+regk*torch.sum(torch.pow(next(detector.get_reg_params()),2))
        errors.append(l.item())

        if i > n_iter/6 and l.item() > 0.6:
          print("STOP DUE TO HIGH LOSS")
          return None, errors
